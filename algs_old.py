...
def tiresias_las(jobs, total_gpus, state, thres=3200, starvation_knob=None, relaxed=False):
    ...
    for m in jobs:
        ...
        if starvation_knob is not None and (m.gpus == 0 or m.preempted_time == INF) and (m.tiresias_starving or m.next_event_time <= m.current_time):
            # Schedule starving jobs
            pos_pri.append(m)
        elif m.tiresias_evicted or (m.current_time - m.preempted_time) * m.gpus >= thres:
    ...
    pos_pri = sorted(pos_pri, key=lambda m: m.evicted_time)
    zero_pri = sorted(zero_pri, key=lambda m: (m.init_sched_time, m.arrival_time))
    neg_pri = sorted(neg_pri, key=lambda m: m.init_sched_time)
    ...
    for m in js:
        if relaxed and gpus > 0:
            ...
            m.schedule(num, (thres + num - 1) / num)
            ...
            if m.gpus == 0 or m.next_event_time <= m.current_time:
                m.schedule(m.philly_request, (thres + m.philly_request - 1) / m.philly_request)
            else:
                m.schedule(m.philly_request, m.next_event_time - m.current_time)
            m.tiresias_starving = False
            m.tiresias_evicted = False
            gpus -= m.philly_request
            # log('SET TIMEOUT (%s): %.1f' % (m.name, m.next_event_time))
            ...
            if m.init_sched_time == INF:
                # This is a job that never have scheduled before
                m.schedule(0)
            elif m.next_event_time <= m.current_time:
                assert(sum([j.philly_request for j in pos_pri]) > total_gpus)
                m.tiresias_starving = True
                m.schedule(0)
            else:
                m.schedule(0, m.next_event_time - m.current_time)
            # if m.gpus != 0:
            #     log('%d:%d(%.0f,%.0f,%.0f,%.0f)%d' % (m.tiresias_id, m.gpus, m.arrival_time, m.init_sched_time, m.preempted_time, m.next_event_time, m.remain_iter))
            #     assert(False)
        else:
            m.schedule(0, (m.current_time - m.preempted_time) * starvation_knob)
            # log('SET TIMEOUT (%s): %.1f' % (m.name, m.next_event_time))
    # log('%d' % js[0].current_time)
    # log('%d] %s %s' % (js[0].current_time,
    #     str(['%d:%d(%.0f,%.0f,%.0f,%.0f)%d' % (m.tiresias_id, m.gpus, m.arrival_time, m.init_sched_time, m.preempted_time, m.next_event_time, m.remain_iter) for m in zero_pri]),
    #     str(['%d:%d(%.0f,%.0f,%.0f,%.0f)%d' % (m.tiresias_id, m.gpus, m.arrival_time, m.init_sched_time, m.preempted_time, m.next_event_time, m.remain_iter) for m in neg_pri])))
    # global _TMP
    # global _CNT
    # if _TMP == js[0].current_time:
    #     _CNT -= 1
    #     if _CNT == 0:
    #         assert(False)
    # else:
    #     _CNT = 20
    #     _TMP = js[0].current_time

def tiresias_debug(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=float('inf'), starvation_knob=None, relaxed=False)

def tiresias_debug2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=3200, starvation_knob=100000, relaxed=False)

def tiresias_01_k1(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.1*3600, starvation_knob=1, relaxed=False)

def tiresias_1_k1(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=1*3600, starvation_knob=1, relaxed=False)

def tiresias_2_k1(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=2*3600, starvation_knob=1, relaxed=False)

def tiresias_5_k1(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=5*3600, starvation_knob=1, relaxed=False)

def tiresias_10_k1(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=10*3600, starvation_knob=1, relaxed=False)

def tiresias_01_k2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.1*3600, starvation_knob=2, relaxed=False)

def tiresias_1_k2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=1*3600, starvation_knob=2, relaxed=False)

def tiresias_2_k2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=2*3600, starvation_knob=2, relaxed=False)

def tiresias_5_k2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=5*3600, starvation_knob=2, relaxed=False)

def tiresias_10_k2(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=10*3600, starvation_knob=2, relaxed=False)

def tiresias_01_k5(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.1*3600, starvation_knob=5, relaxed=False)

def tiresias_1_k5(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=1*3600, starvation_knob=5, relaxed=False)

def tiresias_2_k5(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=2*3600, starvation_knob=5, relaxed=False)

def tiresias_5_k5(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=5*3600, starvation_knob=5, relaxed=False)

def tiresias_10_k5(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=10*3600, starvation_knob=5, relaxed=False)

def tiresias_01_k10(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.1*3600, starvation_knob=10, relaxed=False)

def tiresias_1_k10(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=1*3600, starvation_knob=10, relaxed=False)

def tiresias_2_k10(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=2*3600, starvation_knob=10, relaxed=False)

def tiresias_5_k10(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=5*3600, starvation_knob=10, relaxed=False)

def tiresias_10_k10(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=10*3600, starvation_knob=10, relaxed=False)

def tiresias_001_starve(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.01*3600, starvation_knob=None, relaxed=False)

def tiresias_002_starve(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.02*3600, starvation_knob=None, relaxed=False)

def tiresias_005_starve(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.05*3600, starvation_knob=None, relaxed=False)

def tiresias_01_starve(jobs, total_gpus, state):
    tiresias_las(jobs, total_gpus, state, thres=0.1*3600, starvation_knob=None, relaxed=False)
